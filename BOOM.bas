...
Type Collaborateur
    ...
    DateArret       As Date
    DateReprise     As Date
    Transport       As String      ' col 9 Utilisateurs : "OUI"/"NON"
End Type
...
Function EstEnMaladie(c As Collaborateur, d As Date) As Boolean
    If Not c.EnMaladie Then EstEnMaladie = False: Exit Function
    EstEnMaladie = (d >= c.DateArret And d <= c.DateReprise)
End Function
...
Sub EcrireLigneHorizontale(ws As Worksheet, ligne As Integer, nom As String, _
    ...
    For j = 1 To 7
        ...
        ElseIf cellules(j) = "MALADIE" Then
            cel.Interior.Color = RGB(189, 215, 238)
            cel.Font.Bold = True
            cel.Font.Color = RGB(0, 0, 0)
...
Sub AjouterLigneConsolidation(collab As Collaborateur, d As Date, _
    ...
    Select Case activite
        ...
        Case "MALADIE": ws.Rows(lr).Interior.Color = RGB(197, 217, 241)
...
Sub CalculerCumulsSemaine()
    ...
    For i = 2 To lastRow
        ...
        If activite <> "OFF" And activite <> "CONGE" And activite <> "MALADIE" Then
            nbJ(gIdx) = nbJ(gIdx) + 1
            Dim hNet As Double
...
Sub AjouterLignePlanningCollab(c As Collaborateur, d As Date, entree As String, sortie As String, activite As String)
    ...
    Select Case True
        Case activite = "MALADIE"
            valE = "MALADIE": valS = "MALADIE"
        Case activite = "CONGE"
    ...
    Select Case True
        ...
        Case valE = "MALADIE"
            celE.Interior.Color = RGB(197, 217, 241): celE.Font.Color = RGB(0, 32, 96): celE.Font.Bold = True
            celS.Interior.Color = RGB(197, 217, 241): celS.Font.Color = RGB(0, 32, 96): celS.Font.Bold = True
        Case Left(valE, 2) = "TT"
...
'   9=TRANSPORT (utilise : NON = exclu de PLANNING/ETAT)
'   10=TT | 11=TT D | 12=TT F
...
Function LireCollaborateurs(ByRef collabs() As Collaborateur) As Integer
    ...
    For i = 1 To nb
        ...
        ' col 9 = TRANSPORT
        collabs(i).Transport = UCase(Trim(ws.Cells(i + 1, 9).Value))
...
Sub AppliquerCongesEtTT(ByRef cellules() As String, _
    ...
    For j = 1 To 7
        ...
        If cellules(j) <> "OFF" Then
            If EstEnMaladie(c, d) Then
                ' Maladie prioritaire sur tout
                cellules(j) = "MALADIE"
                entTab(j) = "": sorTab(j) = "": pdTab(j) = "": pfTab(j) = ""
            ElseIf EstEnConge(c, d) Then
...
Sub EcrireLigneAvecConsolidation(ws As Worksheet, ligne As Integer, _
    ...
    Dim j As Integer
    For j = 1 To 7
        If cellules(j) <> "OFF" And cellules(j) <> "CONGE" And cellules(j) <> "MALADIE" Then
            Dim hNet As Double
    ...
    For j = 1 To 7
        ...
        ' Regle TRANSPORT = NON : le collaborateur ne doit pas apparaitre
        ' dans la feuille PLANNING (et donc ne sera pas compte dans les
        ' statistiques ETAT si celles-ci s'appuient sur PLANNING). Les
        ' autres feuilles (feuille projet, CONSOLIDATION, ROTATION...)
        ' continuent de le traiter normalement.
        If c.Transport <> "NON" Then
...
Sub GenererPlanningGOOGLELEADS(collabs() As Collaborateur, nb As Integer)
    ...
    For k = 1 To nbGL
        ...
        For j = 1 To 7
            If EstEnConge(collabs(idx), DateDuJour(j)) Or EstEnMaladie(collabs(idx), DateDuJour(j)) Then
    ...
    For k = 1 To nbGL
        ...
        For j = 1 To 7
            joursConge(j) = EstEnConge(collabs(idx), DateDuJour(j)) Or EstEnMaladie(collabs(idx), DateDuJour(j))
        Next j
        ...
        For j = 1 To 7
            If EstEnMaladie(collabs(idx), DateDuJour(j)) Then
                cellules(j) = "MALADIE"
                entTab(j) = "": sorTab(j) = "": pdTab(j) = "": pfTab(j) = ""
            ElseIf joursConge(j) Then
                cellules(j) = "CONGE"
                entTab(j) = "": sorTab(j) = "": pdTab(j) = "": pfTab(j) = ""
        ...
        For j = 1 To 7
            If cellules(j) <> "CONGE" And cellules(j) <> "OFF" And cellules(j) <> "MALADIE" Then
...
Sub TraiterRenforts(collabs() As Collaborateur, nb As Integer)
    ...
    For r = 2 To lastRow
        ...
        For i = 1 To nb
            ...
            If EstEnMaladie(collabs(i), dateBesoin) Then GoTo NextCollab
            ...
            For p = 2 To lastPlan
                If wsPlan.Cells(p, 5).Value = collabs(i).nomComplet And _
                   CStr(wsPlan.Cells(p, 1).Value) = CStr(semBesoinCalc) Then
                    ...
                    If planEntree <> "OFF" And planEntree <> "CONGE" And planEntree <> "MALADIE" And planEntree <> "" Then
                        dispo = True
                    End If
                    Exit For
                End If
